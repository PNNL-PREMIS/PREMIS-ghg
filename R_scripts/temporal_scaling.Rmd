...
do_sim <- function() {
  ...
  q10 <- 2
  R0 = 1.0
  test <- tibble(Year = 1:100,
                 Temp = dTdt * Year,
                 Resp = R0 * q10 ^ (Temp / 10)) 
  test$Resp_fuzz <- test$Resp * rnorm(nrow(test), mean = 1, sd = median_error)
  
  test$trend_p <- NA
  for(i in seq_len(nrow(test))) {
    if(i > 2) {
      m <- lm(Resp_fuzz ~ Year, data = test[1:i,])
      test$trend_p[i] <- summary(m)$coefficients["Year", "Pr(>|t|)"]
    }
  }
  test
}
...
results %>% 
  ...
  results_summary
qplot(Year, trend_p, data = results_summary, color = trend_p < 0.05) +
