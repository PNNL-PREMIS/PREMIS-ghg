...
Continuous data: `r nrow (licor_data)` observations; `r removed` (`r round(removed / nrow(licor_data) * 100, 0)`%) removed because of chamber closure or other problems. Final data is `nrow(licor_data_clean)` observations.

Continuous data: `r nrow (cld)` observations; `r removed_cld` (`r round(removed_cld / nrow(cld) * 100, 0)`%) removed because of chamber closure or other problems. Final data is `nrow(cld_clean)` observations.
...
p <- ggplot(lds_plot, aes(x = mean_gt_2, y = value, color = variable)) + 
  ...
  scale_color_viridis_d("Samples", labels = c("1x", "2x")) +
...
ggsave("temporal_figures/FigureS1-hourly_pacf_by_month.png", plot = p)
```
...
p <- ggplot(smry, aes(Seconds, CV, label = Labels, group = Labels, color = Labels == "1 year")) +
  ...
  geom_text(aes(label = N), size = 2.5, nudge_y = -0.05) +   # N info
