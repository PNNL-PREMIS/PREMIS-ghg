...
What's driving times of high CV, i.e. when Rs changes a lot hour to hour?
...
ggplot(cld_cv, aes(T5_diff, SMoist_diff, color = CV)) +
  ...
  ggtitle(paste("F. Drivers of hourly CV, ", 
                excluded_count, "of", nrow(cld_cv), "observations not shown"))
...
ggplot(cld_24hr_errors, aes(Hour, Diff_24hr_rel, group = Month, color = Month)) +
  geom_hline(yintercept = 0, linetype = 2) +
  geom_line() +
  ylab("Difference from 24-hour mean flux") + 
  ggtitle("J. Best time of day") +
...
How often do we need to sample to get good monthly number? (Use hourly data.) We use the Student's t statistics and R's `power.t.test()` function to calculate this based on the standard deviation of hourly Rs, the desired power of the test, and the allowable delta (difference from the true mean value).

```{r sample-size, echo=FALSE}
# Helper function
sample_n <- function(flux, delta, power) {
  suppressWarnings({
    out <- try(power.t.test(delta = delta, sd = sd(flux), power = power,
                            sig.level = 0.05, type = "one.sample",
                            alternative = "two.sided"), silent = TRUE)
  })
  if(class(out) == "try-error") { NA } else { out$n }
}

results_list <- list()
for(delta in c(0.25, 0.50, 0.75, 1)) {
  for(power in seq(0.05, 0.9, by = 0.05)) {
    cld_clean %>%
      ...
      summarise(n = sample_n(Flux, delta, power)) %>% 
      mutate(delta = delta, power = power) ->
      results_list[[paste(delta, power)]]
  }
}
bind_rows(results_list) %>% 
  group_by(delta, power) %>% 
  summarise(n_sd = sd(n), n = mean(n)) %>% 
  ungroup %>% 
...
A table of required sample sizes to estimate monthly means for various delta (tolerance) values, assuming beta = `r table_power`:
...
results_list <- list()
for(delta in c(0.25, 0.50, 0.75, 1)) {
  for(m in unique(month(cld_clean$Timestamp))) {
    cld_clean %>%
      filter(month(Timestamp) == m,
             !is.na(Flux)) %>% 
      group_by(Port) %>%    # no year,month
      summarise(n = sample_n(Flux, delta, power = table_power)) %>% 
      mutate(delta = delta, Month = m) ->
      results_list[[paste(m, delta)]]
  }
}
bind_rows(results_list) %>% 
  group_by(Month, delta) %>% 
  summarise(entry = paste(round(mean(n), 1), "±", round(sd(n), 1))) %>% 
  ungroup %>% 
  spread(delta, entry) %>% 
...
ggplot(srdb, aes(x = CV)) + geom_histogram(bins = 20) +
  ...
  ggtitle(paste("N. Interannual CV, N =", nrow(srdb)))

p1 <- ggplot(filter(srdb, !is.na(MI_category)), aes(MI_category, CV)) + 
  scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
  geom_violin() + geom_jitter() +
  xlab("Measurement interval (days)") + ylab("Interannual CV") +
  ggtitle("O. Effect of measurement interval and annual coverage")

p2 <- ggplot(filter(srdb, !is.na(AC_category)), aes(AC_category, CV)) + 
  scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
  geom_violin() + geom_jitter() +
...
distributions <- bind_rows(tibble(Labels = "1 minute", CV = meas_error_1$CV),
                           ...
                           tibble(Labels = "1 day", CV = cld_cv_daily$CV),
...
* The what's driving hour-to-hour CV analysis needs work; maybe remove entirely, and just report what drives the flux itself. Seems clearer
* Investigate/remove screwy survey collar
* Could do a real Monte Carlo sampling exercise...i.e. looking for most effective ways to reduce error. Not sure there's enough data for this though.
