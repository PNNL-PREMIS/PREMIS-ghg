...
for(p in unique(cld_clean$Port)) {
  ...
  cld_p$CV <- NA_real_
  for(i in seq_len(nrow(cld_p))[-1]) {
    obs <- c(cld_p$Flux[i-1], cld_p$Flux[i])
    cld_p$CV[i] <- sd(obs) / mean(obs)
  }
  cld_cv[[p]] <- cld_p
}
...
ld_monthly$CV <- NA_real_
results <- list()
for(collar in unique(ld_monthly$Collar)) {
  ldmc <- filter(ld_monthly, Collar == collar)
  for(i in seq_len(nrow(ldmc))[-1]) {
    obs <- c(ldmc$Flux[i-1], ldmc$Flux[i])
    ldmc$CV[i] <- sd(obs) / mean(obs)
  }
  results[[collar]] <- ldmc
}

lmd_cv <- bind_rows(results)
