...
ggplot(lds_plot, aes(x = mean_gt_2, y = value, color = variable)) + 
  ...
  geom_point() +
  scale_color_discrete("Samples", labels = c("1x", "2x")) +
...
save_plot("temporal_figures/Figure1-sampling.png", plot = last_plot())
...
save_plot("temporal_figures/Figure2-hourly_pacf.png", plot = last_plot())
```
...
save_plot("temporal_figures/Figure4-daily_pacf.png", plot = last_plot())
```
...
save_plot("temporal_figures/Figure3-time_of_day.png", plot = last_plot())
```
...
for(delta_fraction in c(0.05, 0.1, 0.25, 0.50)) {
  for(power in seq(0.05, 0.95, by = 0.05)) {
    cld_clean %>%
      ...
      summarise(n = sample_n(Flux, 
                             delta = delta_fraction * mean(Flux), 
                             power)) %>% 
...
print(p)
save_plot("temporal_figures/Figure5-annual_effect.png", plot = p)
...
ggplot(smry, aes(Seconds, CV, label = Labels, group = Labels)) +
  ...
  ggtitle("P. Summary")
save_plot("temporal_figures/Figure6-summary.png", plot = last_plot())
