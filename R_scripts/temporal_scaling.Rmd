...
save_agu_plot <- function(fn, plot = last_plot(), ...) {
  dir <- "~/Desktop/agu_slides/"
  if(!dir.exists(dir)) dir.create(dir)
  suppressMessages(ggsave(file.path(dir, fn), plot, ...))
}
...
srdb %>% 
  ...
  ggplot(aes(x = Rs_interann_cv)) + geom_histogram(bins = 30) +
  scale_x_continuous(labels = scales::percent) +
  xlim(c(0, 1.5)) +
  geom_vline(xintercept = median_interann_cv, color = "red") +
  xlab("CV") + ylab("CV between successive years")
...
ggplot(meas_error_1, aes(x = cv)) + geom_histogram(bins = 30) +
  ...
  xlab("CV") + ylab("CV between successive IRGA measurements")
...
co2_fuzz_df$p <- out$signif[co2_fuzz_df$lat, co2_fuzz_df$lon]

ggplot(sample_frac(co2_fuzz_df, 0.01), aes(year, flux, group = paste(lat, lon))) + geom_line()
```
...
trend_emergence <- function(rd) {
  Year <- seq_len(length(rd))
  trend_p <- rep(NA, length(rd))
  for(i in seq_along(trend_p)) {
    if(i > 2) {
      m <- lm(rd[1:i] ~ Year[1:i])
      trend_p[i] <- summary(m)$coefficients[2, "Pr(>|t|)"]
    }
  }
  trend_p
}
...
p <- ggplot(respdata, aes(Year, Resp)) + 
  ...
  ylab("Respiration") + ylim(c(0.75, 1.5)) + 
...
do_sim <- function(respdata, error = 0.0) {
...
library(parallel)
n_sims <- 100
for(i in seq_len(n_sims)) results[[i]] <- do_sim(respdata, error = median_error)

#results <- mclapply(1:100, do_sim, respdata)
