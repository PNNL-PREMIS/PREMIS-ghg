...
cld_clean %>% 
  group_by(Year = year(Timestamp), Yday = yday(Timestamp), Port) %>% 
  summarise(SMoist = mean(SMoist),
            T5 = mean(T5)) %>% 
  left_join(q10_results, by = c("Year", "Yday", "Port")) %>% 
  filter(estimate < 10) ->
...
mainplots <- function(q10_results_clean, timescale) {
  ...
  p4 <- ggplot(q10_results_clean, aes(SMoist, estimate)) +
    geom_point() + facet_wrap(~term) +
    ggtitle(timescale)
  print(p4)
  
  p5 <- ggplot(q10_results_clean, aes(T5, estimate)) +
    geom_point() + facet_wrap(~term) +
    ggtitle(timescale)
  print(p5)
}
...
cld_clean %>% 
  ...
  summarise(SMoist = mean(SMoist),
            T5 = mean(T5)) %>% 
  left_join(q10_results, by = c("Year", "Month", "Port")) %>% 
  filter(estimate < 10) ->
