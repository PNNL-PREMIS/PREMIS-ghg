...
control_cols %>% 
  ...
  mutate(CV = sd(Flux)/mean(Flux))

```
# Results

### *Basic results*
We measured Rs, soil temperature, and soil moisture on `r length(unique(control_cols$Date))` different days across the one-year period (Figure 2). Soil temperatures ranged from `r min(flux_data$T5)` to `r round(max(flux_data$T5), digits = 1)` (at 5 cm) and `r min(flux_data$T20)` to `r max(flux_data$T20)` (at 20 cm); volumetric soil moisture values were `r round(min(flux_data$SMoisture), digits = 2)`-`r round(max(flux_data$SMoisture), digits = 2)`. Fluxes ranged from `r round(min(flux_data$Flux), digits = 2)` µmol m-2 s-1 (in March 2019) to `r round(max(flux_data$Flux), digits = 2)` µmol m-2 s-1 (in July 2018). The coefficient of variability between collars within plots, a measure of spatial variability, ranged from 42% to 72%. This implied that a large number of samples was required to estimate Rs accurately (Table 2) .

There was large variability in the basal area and number of trees close to the measurement collars (Figure 3). Mean values were 0 trees (with a basal area of 0 m2) within 1 m, 5 trees (0.29 m2) within 5 m, and 19 trees (1.11 m2) at 10 m. At our maximum radius of measurement, 15 m, there were on average 40 trees and 2.33 m2 of basal area, ranging from a minimum of 0.00 m2 to a maximum of 3.19 m2. The forest was thus highly spatially variable in its distribution of trees relative to the Rs measurement collars. 
