...
model_stats <- function(mods, dat, which) {
  ...
  p <- ggplot(dat, aes(x = BA_m2, y = no_BA_residuals, color = Site)) + 
    ...
    ylab(expression(Residual~(µmol~m^-2~s^-1))) +
...
# ...and plot
pal <- (wes_palette(name = "Cavalcanti1", 4, type = "discrete"))

p <- BA_dat_complete %>% 
  ggplot(aes(x = dist, y = BA_m2, group = Collar, color = n_trees)) + 
#  scale_colour_gradientn(colours = pal) +
    scale_color_jcolors_contin(palette = rev("pal10")) +
...
fig2 <- ggdraw() +
  draw_plot(p + geom_line(size = 1.5, na.rm = TRUE, alpha = 0.7)) +
