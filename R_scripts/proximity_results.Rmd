...
fig2 <- ggdraw() +
  draw_plot(p + geom_line(size = 0.5, na.rm = TRUE)) +
...
tabledata %>% 
  transmute(Salinity = Salinity,
            `Trees (/ha)` = paste0(tree_mean, " (", tree_sd, ")"),
            `BA (m2/ha)` = paste0(BA_mean, " (", BA_sd, ")")) %>%
...
licor_daily_data %>% 
  filter(!is.na(meanFlux), !is.na(sdFlux), Group == "Control") %>%
  ggplot(aes(x = yday(Timestamp), y = meanFlux, group = Collar)) +
  geom_point() +
