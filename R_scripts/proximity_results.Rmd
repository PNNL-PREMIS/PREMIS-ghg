...
run_model <- function(flux, T5, T20, SM, BA, site, model_type) {
  ...
  dat <- tibble(flux, T5, T20, SM, BA, site)
  if(any(is.na(dat))) {
    stop("Can't have any NAs in run_model data")
  }
  f <- formula(log(flux) ~ T5 * BA + T20 * BA + SM + I(SM ^ 2) + BA)
  f_no_BA <- formula(log(flux) ~ T5 + T20 + SM + I(SM ^ 2))
  
  if(model_type == "linear") {
    mod_full <- lm(formula = f, data = dat)
    mod_no_BA <- lm(formula = f_no_BA, data=dat)
  } else if(model_type == "lme") {
    mod_full <- nlme::lme(f, data = dat,
                          # allow for random variation across sites
                          random = ~ 1 | site,
                          method = "ML") # because we're doing stepAIC below
    mod_no_BA <- nlme::lme(f_no_BA, data = dat, random = ~ 1 | site, method = "ML")
  } else {
...
model_stats <- function(mods, dat) {
  ...
  p <- ggplot(dat, aes(no_BA_residuals, BA_m2)) + 
    geom_point(aes(color = Dest_Salinity)) + geom_smooth(method = "lm")
  print(p)
...
growing_flux <- filter(control_cols_5m, Growing_season) %>% filter_NAs
...
dormant_flux <- filter(control_cols_5m, !Growing_season) %>% filter_NAs
#print(dormant_flux)
...
## Filter true controls for the bottom and top 50% soil moisture values
...
#print(m_high)
h3h <- with(m_high, run_model(Flux, T5, T20, SMoisture, BA_m2, site = Dest_Salinity, model_type = MODEL_TYPE))
...
for (i in seq.int(2, max(tree_prox_data$Distance_m, na.rm = TRUE))) {
  ...
  data <- left_join(control_cols, BA, by = "Collar") %>%
    select(Collar, Flux, T5, T20, SMoisture, BA_m2, Dest_Salinity) %>%
    filter_NAs
