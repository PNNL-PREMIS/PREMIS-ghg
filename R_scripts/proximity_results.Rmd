...
prox_model <- function(flux, T5, T20, SM, BA, site) {
  # Fit model
  #mod_full <- lm(log(flux) ~ T5 + T20 + SM + I(SM ^ 2) + BA)  # temporary
  #mod_full <- lme4::lmer(log(Flux) ~ T5 + T20 + SMoisture + I(SMoisture ^ 2) + (BA_m2 | site))
...
h1 <- with(control_cols_5m, prox_model(Flux, T5, T20, SMoisture, BA_m2, site = Dest_Salinity))
summary(h1)
anova(h1)
plot(h1)
...
for (i in 2:max(tree_prox_data$Distance_m)) {
  ...
  model <- with(data, prox_model(Flux, T5, T20, SMoisture, BA_m2, site = Dest_Salinity))
  smry <- anova(model)
