...
control_cols_5m %>% 
  filter(BA_m2 > BA_CUTOFF) %>% mutate(which = "High BA") -> high_BA

control_cols_5m %>% 
  filter(BA_m2 <= BA_CUTOFF) %>% mutate(which = "Low BA") -> low_BA

rbind(high_BA, low_BA) -> BA
  
ggplot(data = BA, aes(x = yday(Timestamp), y = Flux, color = which)) +
