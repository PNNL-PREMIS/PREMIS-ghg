...
model_stats <- function(mods, dat) {
  ...
  p <- ggplot(dat, aes(no_BA_residuals, BA_m2, color = Site)) + 
    ...
    scale_color_discrete(labels = c("GCREW", "Canoe Shed", "North Branch"))
  print(p)
  
  m_reduced <- mods$reduced_model
  
  print(summary(m_reduced))
  print(car::Anova(m_reduced, type = "III"))
  plot(m_reduced)
}
...
for(delta in c(0.25, 0.50, 0.75, 1)) {
  for(power in seq(0.1, 0.9, by = 0.1)) {
    licor_daily_data %>%
      filter(!is.na(meanFlux)) %>% 
      group_by(Dest_Salinity, Dest_Elevation) %>%
      summarise(n = sample_n(meanFlux, delta, power)) %>% 
