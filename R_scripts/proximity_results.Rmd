...
library(readr)
# Load data
BASE <- "/Users/penn529/Documents/GitHub/PREMIS-ghg"
licorDat <- get(load(file.path(BASE, "outputs", "licordat.rda")))
treeProxDat <- read_csv(file.path(BASE, "inventory_data", "collar_to_tree_prox.csv"))
...
control_cols <- filter(licorDat, Experiment == "Control" & is.na(Notes) & Dest_Salinity == "High")
BA5 <- basal_area_sum(treeProxDat, 5)
control_cols <- left_join(control_cols, BA5, by = "Collar")

#p1 <- prox_model(control_cols$Flux, control_cols$T5, control_cols$T20, control_cols$SMoisture, control_cols$BA_m2, #control_cols$Destination_Plot)
