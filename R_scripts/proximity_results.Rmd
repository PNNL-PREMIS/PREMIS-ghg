...
prox_model <- function(flux, T5, T20, SM, BA, site, model_type) {
  if(model_type == "linear") {
    mod_full <- lm(log(flux) ~ T5 * BA + T20 * BA + SM + I(SM ^ 2) + BA)
...
# # This function takes prox_model() objects and produces summaries and plots
# model_stats <- function(mods) {
#   
#   mods <- with(data, 
#              prox_model(Flux, T5, T20, SMoisture, BA_m2, site = Dest_Salinity, model_type = model_type))
...
prox_data %>%
  ...
  mutate(DBH_cm = if_else(is.na(Tag), No_tag_DBH, DBH_cm),
         ...
         BA_sqm = (DBH_cm / 100 / 2) ^ 2 * pi) ->  # from DBH (cm) to area (m2)
  tree_prox_data
```
...
tree_data %>% 
  ...
  summarise(n = n(), 
            ...
            `BA (m2/ha)` = sum((DBH_cm / 100 / 2) ^ 2 * pi, na.rm = TRUE) / 
...
tabledata %>% 
  ...
  knitr::kable(digits = 1, format = "html", col.names = c("Site", "Salinity", "Trees (/ha)", expression(BA~(m^2~ha^-1)), "Soil Type"))
...
licor_daily_data %>% 
  ...
  labs(x = "Date", 
       y = expression(Flux~(Âµmol~CO[2]~m^-2~s^-1))) +
...
# ===============================================================================
# TEMPORARY: at this point we add in fake data, because right now (January 2019)
# there's only GCREW proximity data available.  This means we can't run the
# mixed-effects model, which needs multiple sites (as the random effect is 
# site/salinity level). So generate some fake data for the other sites. -BBL
add_fake_data <- function(dat) {
  dat$FAKE <- FALSE
  m <- lm(BA_m2 ~ Flux, data = dat)
  dat %>% 
    filter(is.na(BA_m2)) %>% 
    # predict BA from flux
    mutate(BA_m2 = predict(m, newdata = data.frame(Flux = Flux)),
           # add some random noise
           BA_m2 = BA_m2 + rnorm(n = n(), sd = 2 * sd(BA_m2)),
           FAKE = TRUE) %>% 
    bind_rows(filter(dat, !is.na(BA_m2)))
} 

control_cols_5m <- add_fake_data(control_cols_5m)
# ===============================================================================

# Run the main analysis
h1 <- with(control_cols_5m, prox_model(Flux, T5, T20, SMoisture, BA_m2, site = Dest_Salinity, model_type = "linear"))
...
h2g <- with(growing_flux, prox_model(Flux, T5, T20, SMoisture, BA_m2, site = Dest_Salinity, model_type = "linear"))
...
print(dormant_flux)
h2d <- with(dormant_flux, prox_model(Flux, T5, T20, SMoisture, BA_m2, site = Dest_Salinity, model_type = "linear"))
...
SMOISTURE_MEDIAN <- median(control_cols_5m$SOILMOISTURE)

# Filter true controls for the bottom and top 50% soil moisture values; this will be our "limited" and "ample" moisture times
m_limited <- filter(control_cols_5m, SOILMOISTURE < SMOISTURE_MEDIAN)
print(m_limited)
h3l <- with(m_limited, prox_model(Flux, T5, T20, SMoisture, BA_m2, site = Dest_Salinity, model_type = "linear"))

#model_stats(h3l)

m_ample <- filter(control_cols_5m, SOILMOISTURE >= SMOISTURE_MEDIAN)
print(m_ample)
h3a <- with(m_ample, prox_model(Flux, T5, T20, SMoisture, BA_m2, site = Dest_Salinity, model_type = "linear"))
...
for (i in seq.int(2, max(tree_prox_data$Distance_m))) {
  ...
  data <- add_fake_data(data)  # TEMPORARY. See note above.
  
  mods <- with(data, prox_model(Flux, T5, T20, SMoisture, BA_m2, site = Dest_Salinity, model_type = MODEL_TYPE))
