...
x$Soil_Type[1] <- "Collington-Wist complex; Collington and Annapolis soils"
x$Soil_Type[2] <- "Annapolis fine sandy loam"
x$Soil_Type[3] <- "Collington and Annapolis soils; Collington, Wist, and Westphalia soils"
...
MODEL_TYPE <- "linear"
mods <- with(control_cols_5m, 
           prox_model(Flux, T5, T20, SMoisture, BA_m2, site = Dest_Salinity, model_type = MODEL_TYPE))
m_reduced <- mods$reduced_model
summary(m_reduced)
anova(m_reduced)
plot(m_reduced)
control_cols_5m$prediction <- predict(m_reduced)
ggplot(control_cols_5m, aes(Timestamp, Flux)) + 
  geom_point() + 
...
h2g <- with(growing_flux, prox_model(Flux, T5, T20, SMoisture, BA_m2, Site, model_type = MODEL_TYPE))

# Filter true controls for dormant season
dormant_flux <- filter(control_cols_5m, !Growing_season)
print(dormant_flux)
h2d <- with(dormant_flux, prox_model(Flux, T5, T20, SMoisture, BA_m2, Site, model_type = MODEL_TYPE))
