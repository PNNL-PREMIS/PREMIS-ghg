...
litter_data %>% group_by(Date_collected, Plot, Litter_Type) %>% 
  summarise(Mass_g = mean(Mass_g)) %>% 
  ggplot(aes(x = Date_collected, y = Mass_g, fill = Litter_Type)) + 
  facet_wrap(~Plot) + geom_area(aes(color = Litter_Type)) +
