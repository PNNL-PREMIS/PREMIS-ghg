...
licor_7810 <- readd('licor_7810_data', path = PATH)
...
licor_daily_data %>% 
  ...
  summarise(Timestamp = mean(Timestamp), 
            sd_CO2_flux = sd(mean_CO2_flux), 
            mean_CO2_flux = mean(mean_CO2_flux), 
            meanSM = mean(meanSM)) %>% 
  ungroup %>% 
...
daily_treatment_means %>% 
  ...
  do(data.frame(Date = mean(.$Date),
                transplant_effect = tryCatch(anova(lm(mean_CO2_flux ~ Origin_Plot, data = .))$`Pr(>F)`[1],
                                             error = function(e) NA))) %>% 
  ungroup() %>% 
...
p <- daily_treatment_means %>% 
  filter(!is.na(mean_CO2_flux), !is.na(sd_CO2_flux), mean_CO2_flux > 0) %>% 
  ggplot(aes(x = Timestamp, y = mean_CO2_flux, color = Experiment, group = Group)) +
  geom_errorbar(aes(ymin = mean_CO2_flux - sd_CO2_flux, ymax = mean_CO2_flux + sd_CO2_flux)) +
  geom_point() +
...
daily_treatment_means %>% 
  filter(Experiment=="Control", mean_CO2_flux > 0) %>% 
  select(Date, Destination_Plot, ControlGroup, mean_CO2_flux) %>% 
  mutate(Year = as.factor(year(Date))) %>% 
  pivot_wider(names_from = ControlGroup, values_from = mean_CO2_flux) %>% 
...
### CH4
```{r ch4, echo = FALSE, warning = FALSE}
licor_7810 %>% 
  mutate() %>% 
  ggplot(aes(x = temp_soil, y = log(CH4_flux))) + 
  geom_point()
...
licor_daily_data %>% 
  filter(!is.na(meanT20), !is.na(mean_CO2_flux)) %>% 
  ggplot(aes(x = meanT20, y = mean_CO2_flux, color = Dest_Elevation)) +
...
licor_data %>% 
  ...
  summarise(CV = sd(CO2_flux) / mean(CO2_flux), 
            n = n(), 
...
readd("tree_data", path = PATH) %>% 
  group_by(Salinity, Elevation, Species) %>% 
  summarise(n = n()) -> tree_data
  
ggplot(tree_data, aes(fill = Species, values = n)) +
